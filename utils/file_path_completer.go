...
func (c *FilePathCompleter) Complete(d prompt.Document) []prompt.Suggest {
	...
	var path string
	parsedCmd := ParseCmd(d.TextBeforeCursor())
	if c.ParseWholeLine {
		path = d.TextBeforeCursor()
	} else {
		path = parsedCmd[len(parsedCmd)-1]
	}
...
func printErr(err error) {
	color.New(color.FgRed, color.Bold).Println("An error occurred while trying to provide completeions!")
